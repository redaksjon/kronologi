...
export async function generateContent(config: AnalysisConfig, parameters: Parameters, mindshahnConfig: MindshahnConfig): Promise<Section<Content>> {
    ...
    for (const [key, value] of Object.entries(config.content)) {
        ...
        if (value.type === 'summary') {
            directoryPath = join(mindshahnConfig.summaryDirectory, value.directory || '', year.toString(), month.toString());
        } else {
            directoryPath = join(mindshahnConfig.activityDirectory, value.directory || '', year.toString(), month.toString());
        }
